...
const MUTANTS = [
    ...
    ['layouts.js', 'layouts.test.mjs',
        ...
        'const raw = prevEdge + Math.round(usable * weights[i] / total);',
        'round each slice independently (leaves seams)'],
    ['layouts.js', 'layouts.test.mjs',
        'Math.max(raw, prevEdge + MIN_TILE),\n            usable - remaining * MIN_TILE)',
        'raw, usable)',
        'stop clamping the edge to a feasible band (overlap)'],
    ['layouts.js', 'layouts.test.mjs',
    ...
    ['arranger.js', 'arranger.test.mjs',
        '(maximized || win.allows_resize()) &&', 'win.allows_resize() &&',
        'judge a maximized window by allows_resize(), which is always false'],
    ['arranger.js', 'arranger.test.mjs',
